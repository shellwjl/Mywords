...
class APIClient(object):
    ...
    def getAccount(self):
        url=self.api_url+'/account/?access_token=%s' % self.access_token
        resp=urllib.request.urlopen(url)
        body=resp.read().decode("utf-8")
        r=json.loads(body)
        return r
    def getAccount1(self,access_token):
        url=self.api_url+'/account/?access_token=%s' % access_token
        resp=urllib.request.urlopen(url)
        body=resp.read().decode("utf-8")
        r=json.loads(body)
        return r
    def getWordExp(self,word):
        url=self.api_url+'/bdc/search/?word=%s'% word
        resp=urllib.request.urlopen(url)
        body=resp.read().decode("utf-8")
        r=json.loads(body)
        return r
    def getSentences(self,wordid,access_token):
        url=self.api_url+'/bdc/example/?vocabulary_id=%s &type=sys&access_token=%s'%(wordid,access_token)
        resp=urllib.request.urlopen(url)
        body=resp.read().decode("utf-8")
        r=json.loads(body)
        return r
    def getNotes(self,wordid,access_token):
        url=self.api_url+'/bdc/note/?vocabulary_id=%s&access_token=%s' %(wordid,access_token)
        resp=urllib.request.urlopen(url)
        body=resp.read().decode("utf-8")
        r=json.loads(body)
        return r

    def addnotes(self,wordid,usernotes,access_token):
        data = urllib.parse.urlencode({'vocabulary': wordid, 'note': usernotes,'access_token':access_token})
        data = data.encode('utf-8')
        request = urllib.request.Request("https://api.shanbay.com/bdc/note/")
    ...
    def getSynonyms(self,word):
        synonymlist=[]
        url="http://www.collinsdictionary.com/dictionary/english-thesaurus/%s" % word
        response=urllib.request.urlopen(url)
        soup = BeautifulSoup(response)
        synonyms=soup.a=soup.find_all("a",class_="xr_ref_link")
        for synonym in synonyms:
            synonymlist.append(synonym.get_text())
